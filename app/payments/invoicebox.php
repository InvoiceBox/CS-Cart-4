...
if (defined('PAYMENT_NOTIFICATION')) {
    ...
    $participantOrderId = isset($_REQUEST['participantOrderId']) ? (int)$_REQUEST['participantOrderId'] : false;
    ...
    $processor_data = isset($processor_data) ? $processor_data : [];
    $order_info = isset($order_info) ? $order_info : [];
    ...
    $total_amount = 0;
    $it = 0;
    $payment_desc = 'Заказ №' . $order_id . ' - ' . $order_info['total'] . ' ' . $order_info['secondary_currency'];
    ...
    if (!empty($order_info['products'])) {
        foreach ($order_info['products'] as $k => $v) {
            ...
            $form_data["itransfer_item{$it}_type"] = 'commodity';
        }
    }
    if ($sh_cost > 0) {
        $it++;
        ...
        $form_data["itransfer_item{$it}_measure"] = 'шт.';
        $form_data["itransfer_item{$it}_type"] = 'service';
        //$form_data["itransfer_item{$suffix}_vat"] = $vat;
    }
